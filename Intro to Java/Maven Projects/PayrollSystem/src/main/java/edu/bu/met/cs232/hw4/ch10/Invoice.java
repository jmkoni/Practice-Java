 ...
 *
 * This class represents an invoice that needs to be paid. Since it implements Payable,
 * it must also implement getPaymentAmount. It requires the following parameters:
 *   * partNumber: String
...
public class Invoice implements Payable {
  ...
  {
    return String.format("%s:%n%s: %s (%s)%n%s: %d%n%s: $%,.2f%n", "invoice", "part " +
        "number", getPartNumber(), getPartDescription(), "quantity", getQuantity(),
